...
UINT16
CalculateICMPv6Checksum(struct in6_addr srcAddr,
                        ...
                        BOOLEAN isSource, BOOLEAN isTx)
{
    ...
    if (memcmp(addrField, &newAddr, sizeof(struct in6_addr))) {
        ...
        OVS_LOG_INFO("Nat to new address.");
        ...
        *addrField = newAddr;
        ...
        if (layers->isIcmp) {
            ...
            icmp->checksum = 0x00;
            icmp->checksum = CalculateICMPv6Checksum(ipHdr->saddr, ipHdr->daddr,
                                                     ntohs(ipHdr->payload_len),
                                                     IPPROTO_ICMPV6,
                                                     (uint16_t *)((uint8_t *)icmp),
                                                     ntohs(ipHdr->payload_len));
            OVS_LOG_INFO("Nat icmp packet, recompute checksum.");
        }
    }
                    ...
                    INT actionsLen)
{
    ...
    if (layers->isIPv6) {
        ...
        OVS_LOG_INFO("+OvsDoExecuteActions , %s:%d->%s:%d", src, ntohs(key->ipv6Key.l4.tpSrc),
